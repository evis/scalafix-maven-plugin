...
import io.github.evis.scalafix.maven.plugin.params.{ConfigParam, ModeParam, SourceDirectoryParam}
  ...
  "run()" should "succeed if no issues found in project" in {
    ...
    val params =
      List(
        ...
        ConfigParam(new File("src/test/resources/project/.success.scalafix.conf")),
        ModeParam(CHECK)
      )
